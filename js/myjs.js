...
$(function(){
    ...
    //FUNCTIONS
        ...
        this.right = function(){
            ...
            //cycle for each row on the game field(this.field)
            for(var rowNum = 0; rowNum < this.field.length; ++rowNum){

                //write to statistic array
                //moved and raised
                
                //count of cells that will raised(used for blocking double raising the same cell)
                var tokenRaiseShift = 0;
                //indicated if in this iteration of cycle was rasing of the token
                var tokenRaised = false;
                
                var from;
                var to;
                //try to catch the token(from right to left)
                for(var i = (this.field[rowNum].length - 1); i >= 0; --i){
                    if(this.field[rowNum][i] === 0){
                        continue;
                    }
                    else{
                        //saving start and end coordinates
                        from = {x:i,y:rowNum};
                        to = {x:i,y:rowNum};
                        //refreshing raised bool before moving
                        tokenRaised = false;
                        //start moving token(to the right)
                        for(var j = (i + 1); j < (this.field[rowNum].length - tokenRaiseShift); ++j){
                            //if cell is empty than moving on this position
                            if(this.field[rowNum][j] === 0){
                                to.x = j;
                            }
                            //if cell is not equal and not empty than stop the moving
                            else if((this.field[rowNum][j] !== this.field[rowNum][i])){
                                break;
                            }
                            //if cell is raised than moving on this position and raise 
                            else{
                                //write to statistic array
                                //raised and moved
                                tokenRaised = true;
                                //blocking the token for double raising
                                ++tokenRaiseShift;
                                //this.lastTurnDataChange.raised.push({x:j,y:rowNum,value:this.field[rowNum][j]});
                                this.field[rowNum][j] *= 2;
                                this.lastTurnDataChange.raised.push({x:j,y:rowNum,value:this.field[rowNum][j]});
                                to.x = j;
                                this.score += this.field[rowNum][j];
                                this.field[rowNum][from.x] = 0;
                                this.lastTurnDataChange.moved.push({fromx:from.x,fromy:from.y,tox:to.x,toy:to.y});
                                //exit from cycle(stop moving) and catching another token(from right to left)
                                break;
                            }
                        }
                        //collate if token moved(for event when wasn't raising)
                        if(((from.x !== to.x) || (from.y !== to.y)) && (!tokenRaised)){
                            //write to statistic array
                            //moved
                            this.field[rowNum][to.x] = this.field[rowNum][from.x];
                            this.field[rowNum][from.x] = 0;
                            this.lastTurnDataChange.moved.push({fromx:from.x,fromy:from.y,tox:to.x,toy:to.y});
                        }
